...
public class Product {
    ...
    //test
    public Product(String n, int fp, int cp, int tp, String dp, double pr) {
        name = n;
        floorPrice = fp;
        ceilingPrice = cp;
        targetPrice = tp;
    ...
    public int getSalesVolumnOffer(){
        int sum = 0;
        for (SolutionOffer so : solutionoffers) {
            sum = sum + so.getSolutionOfferTotal(); // positive and negative values
        }
        return sum;
    }
